    ...
    <div class="col-lg-8">
        <div class="card shadow-sm border-0">
            ...
            <div class="card-header border-bottom">
                <h5 class="mb-0 card-title fw-bold">Edit User: {{ $user->nama }}</h5>
            </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.user.update', $user->iduser) }}" method="POST">
                    ...
                    <h6 class="text-primary mb-3 mt-4">Ubah Password (Opsional)</h6>
                    <p class="small text-muted">Kosongkan field di bawah jika tidak ingin mengubah password.</p>
                            ...
                            @foreach ($roles as $role)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="roles[]" value="{{ $role->idrole }}" id="role_{{ $role->idrole }}" 
                                        {{ in_array($role->idrole, $oldRoles) ? 'checked' : '' }}>
